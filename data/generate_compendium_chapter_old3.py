...
- Supports optional "section_header" items inside the tagged JSON:
  {"kind":"section_header","title":"2007: Earliest Tweets.","created_at":"2007-01-01T00:00:00Z"}
...
def infer_header_dt(title: str, created_at):
    dt = parse_created_at_any(created_at or "")
    if dt:
        return dt
...
def main():
    ...
    for idx, t in enumerate(tagged):
        if t.get("kind") == "section_header":
            ...
            items.append({
                "kind": "section_header",
                "title": title,
                "dt": infer_header_dt(title, t.get("created_at")),
                "order": idx,
            })
            continue
    ...
    def _sort_key(it):
        ...
        return (dt, it.get("order", 0))

    items.sort(key=_sort_key)

    tweet_items = [it for it in items if it.get("kind") == "tweet"]
    kept_count = len(tweet_items)
    featured_count = sum(1 for it in tweet_items if it.get("featured"))
    ...
    for it in tweet_items:
        ...
        if dt:
            ...
            by_year[dt.year] += 1
        for tg in (it.get("tags") or ["unclassified"]):
            tag_counts[tg] += 1
    ...
    current_year = None
    for it in items:
        if it.get("kind") == "section_header":
            title = it.get("title") or ""
            if title:
                parts.append(f'<h2 class="section-divider">{escape(title)}</h2>')
            continue
        ...
        block = [f'<div class="{" ".join(classes)}" data-tweet-id="{escape(it["id_str"])}">']

        if it.get("star"):
            block.append('<div class="tweet-star" title="High engagement">★</div>')

        if is_ascii:
            block.append(f'<div class="tweet-content tweet-text-ascii"><pre>{escape(it["text"])}</pre></div>')
        else:
            block.append(f'<div class="tweet-content">{auto_link_text(it["text"], it.get("raw") or {})}</div>')

        tags_html = render_tag_badges(tags)
        if tags_html:
            block.append(tags_html)

        media_html = render_media_html(it.get("raw") or {}, media_dir)
        if media_html:
            block.append(media_html)

        quote_html = render_quote_boxes(it.get("raw") or {}, tweets_by_id, media_dir)
        if quote_html:
            block.append(quote_html)

        footer_left = escape(it.get("date_short") or "")
        footer_right = f"id {escape(it['id_str'])}"
