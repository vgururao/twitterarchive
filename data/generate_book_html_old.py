...
def render_media(raw):
    """
    For now, just list media URLs as links.
    If you want to switch to local files later, this is the hook.
    """
    parts = []
    entities = (raw or {}).get("extended_entities") or {}
    ...
    for m in media:
        ...
        if media_url:
            parts.append(
                f'<div class="tweet-media">[media: {escape(mtype or "unknown")}] '
                f'<a href="{escape(media_url)}">{escape(media_url)}</a></div>'
            )
